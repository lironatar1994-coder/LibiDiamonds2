  ...
  return (
    ...
    >
      ...
      <div className={`px-0.5 sm:px-1 ${catalog || editorial ? `${ringCatalog ? "catalog-ring-copy pt-2.5" : "pt-3"} text-right sm:pt-4` : `text-center ${compact ? "pt-3 sm:pt-4" : "pt-3 sm:pt-5"}`}`}>
        <h3 className={`font-display leading-snug transition-colors group-hover:text-gilt-deep ${ringCatalog ? "catalog-ring-name text-base sm:text-lg" : compact ? "text-[0.9rem] sm:text-base" : catalog ? "text-base sm:text-lg" : editorial ? "text-base lg:text-lg" : "text-[0.95rem] sm:text-lg"}`}>
          {product.name}
        </h3>
